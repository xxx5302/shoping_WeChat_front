...
Page({
  ...
  canBuyTip: function(){
    ...
    app.util.message(`此商品在您所属${groupInfo.group_name}不可参与`, 'switchTo:/lionfish_comshop/pages/index/index','error');
  },
  ...
  authSuccess: function() {
    ...
    let url = '/lionfish_comshop/pages/goods/goodsDetail?id=' + id + '&community_id=' + community_id + '&scene=' + scene+ '&type=' + this.data.buy_type;
  ...
  goOrder: function() {
    ...
    if (open_man_orderbuy == 1 && this.data.is_just_addcar==0){
      ...
      if (actPrice * 1 * sku_val < man_orderbuy_money){
        wx.showToast({
          title: '满' + man_orderbuy_money + '元可下单！可先加入购物车',
          icon: 'none'
        })
        return false;
      }
    ...
    if (localtown_moneytype_fixed_freemoney >= 0 && is_only_distribution ==1 && this.data.is_just_addcar==0){
          ...
          if (actPrice * 1 * sku_val < localtown_moneytype_fixed_freemoney){
            wx.showToast({
              title: '同城配送商品满' + localtown_moneytype_fixed_freemoney + '元可下单！可先加入购物车',
              icon: 'none'
            })
            return false;
          }
  ...
  onShareAppMessage: function() {
    ...
    var share_path = 'lionfish_comshop/pages/goods/goodsDetail?id=' + goods_id + '&share_id=' + share_id + '&community_id=' + community_id + '&type=' + buy_type;
